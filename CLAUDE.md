...
- `uv run stashfs mount <backing_file> [mountpoint] [--ttl 300] [--force-ttl 18000] [--debug]` — mount. Prompts for password via `getpass`; empty string always maps to slot 0. Auto-unmounts after `--ttl` seconds idle. `--force-ttl` forces an unmount that many seconds after mount *regardless of activity* (default 5 h = 18000 s; `-1` disables it); the inactivity timer is unaffected. Debug log goes to `/tmp/stashfs.log`. (`uv run python -m stashfs.fuse_app ...` still works.)
...
10. **`fuse_app.py`** — FUSE glue. `Stash(fuse.Fuse)` is a thin wrapper that delegates everything to a `Volume`. Handles password prompt, TTL idle check (updates `_ctime` on every VFS call; a background daemon thread started by `start_ttl_watcher` polls `_should_unmount` every `_watch_interval`s and, on expiry, flushes then triggers `auto_unmount` via a child `multiprocessing.Process` running `fusermount -u -z` — lazy so a momentarily busy mount still detaches; the thread, not `getattr`, drives unmount so a fully idle mount still fires), and error conversion to `-EIO`. Exposes `main`/`mount`/`parse_args`. Also contains the `TIME_PAT` (`/<digits>.<digits>`) quirk — `getattr` fakes an empty regular file for matching paths.
